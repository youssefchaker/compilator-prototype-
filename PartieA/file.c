 ...
 * @return numberToken
 */
numberToken getNumber(char digit, int isNegative){
     // initialisation du compteur
     int i=0;
...
token getToken(){  
     ...
     // Reconnaissance des mots-clé et des identificateurs
          ...
          A.properties.identifier.name = (char *)malloc(IDFSIZE*sizeof(char));
          A.properties.keyword.name = (char *)malloc(VARCHARSIZE*sizeof(char));
          
          // Lire tout le mot
          do{
              A.properties.identifier.name[i] = character;
              A.properties.keyword.name[i] = character;
              character = fgetc(fileSrc); // Lire le caractére suivant
              i++;
          }while(isAlphabetic(character) == 1 || isNumeric(character) == 1);
          // il faut poser \0 à la fin du mot
          A.properties.identifier.name[i] = '\0';
          A.properties.keyword.name[i] = '\0';
          /*
          ...
          */
          ...
          while(i < keyword_token_size){
               ...
               if(strcmp(A.properties.identifier.name, keyword_token[i]) == 0){ // Le mot est un mot-clé
                    /* Le nom token : (nameToken) i convertit l'indice par sa position
                     dans la liste d'énumaration nametoken */
                    A.name = (nameToken) i;

                    // Libérer la case mémoire A.properties.identifier.name
                    // Allocation de la mémoire pour sauvegarder le mot

                    free(A.properties.identifier.name);
                    // retourner le token
                    return A;  
               }
               i++;
          ...
          */
          ...
          free(A.properties.keyword.name);
     ...
     // Reconnaissance des nombres
          ...
          A.properties.number = getNumber(character, 0);
          return A;
     }
     ...
     */
          ...
          // sverifier si c'est un opérateur arithmétique "+" "-"
          nameToken symboleToken = (character == '+' ? PLUS_TOKEN : MINUS_TOKEN);
          // sauvegarder le caractère
          char ch[2] = {character,'\0'};
          // Allocation de la mémoire 
          A.properties.oparith.name = (char *)malloc(3*sizeof(char));
          strcpy(A.properties.oparith.name ,ch);
     ...
     // Reconnaissance de =, ==
          ...
          if(character == '='){ // On a le symbole ==
               A.name = OP_EQ_TOKEN;          
               // Allocation de la mémoire pour sauvegarder le mot
               A.properties.oprel.name = (char *)malloc(4*sizeof(char));
               strcpy(A.properties.oprel.name, "==");
               return A;
          }else{ // On a le symbole =
               A.name = ASSIGNMENT_TOKEN;
               char ch[2] = {character,'\0'};
               // Allocation de la mémoire pour sauvegarder le mot
               A.properties.aff.name = (char *)malloc(3*sizeof(char));
               strcpy(A.properties.aff.name, ch);
               //retourner en arriére le curseur pour que ce caractếre ne soit perdu
               ungetc(character, fileSrc);
               return A;
          }
     ...
     // Si on rencontre d'autre caractéres non analysés
     else{
          A.name = ERROR_TOKEN;
          return A;
     }
 ...
 * @param char [VARCHARSIZE][VARCHARSIZE]
 */
void analyseur(char uniteLexicale[VARCHARSIZE][VARCHARSIZE]){
     char ch[VARCHARSIZE];
     ...
     fgets(ch,VARCHARSIZE,fileSrc);
     // extraire le premier élement selon la chaine " "(espace)
     lexeme = strtok(ch, " ");
     ...
     while (i < nbUniteLex) {
          ...
          // decouper le reste de la chaine et la stocker dans le tableau des lexemes
          lexeme = strtok(NULL, " ");

          //tabulation
          for (int j =0; j< 34 - (strlen(uniteLexicale[i])-2); j++){
               printf(" ");
          }
